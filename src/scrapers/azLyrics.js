...
/**
 * Scrapes lyrics from AZLyrics with retry mechanism
 ...
 */
export async function scrapeLyrics(title, artist, proxy, options = {}) {
  const {
    maxRetries = 2,
    navigationTimeout = 40000, // Reduced from 90000
    waitForSelectorTimeout = 20000, // Reduced from 30000
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      // Optimization: Set aggressive resource blocking for faster page loads
      await page.route(
      ...
      await page.route("**/*advertisement*", (route) => route.abort());
      await page.route("**/*ads*", (route) => route.abort());

      // Navigasi ke halaman search dengan timeout yang lebih pendek
      try {
        await page.goto(searchUrl, {
          waitUntil: "domcontentloaded", // Using domcontentloaded instead of networkidle
      ...
      // Search form - Check if we need to submit a search
      const isSearchPage = await page.evaluate(() => {
        return document.querySelector(".search .form-control") !== null;
      });
      ...
      // Menunggu hasil search dengan timeout yang lebih pendek
      try {
        ...
        throw new ScrapingError(
          ...
          ErrorCodes.TIMEOUT_ERROR,
          {
            ...
            timeoutMs: waitForSelectorTimeout,
            originalError: error.message,
          }
        );
      }
      ...
      // Navigasi ke halaman lirik - reduced timeout
      try {
      ...
      // Only retry for timeouts and navigation errors
      if (
        (error instanceof ScrapingError &&
          (error.code === ErrorCodes.TIMEOUT_ERROR ||
            error.code === ErrorCodes.NAVIGATION_ERROR)) ||
        ...
        if (retryCount <= maxRetries) {
          ...
          // Close browser context before retry
          if (browserContext) {
            try {
  ...
  if (lastError) {
    if (!(lastError instanceof ScrapingError)) {
      throw new ScrapingError(
        `Error saat scraping lirik "${title}" oleh "${artist}" setelah ${maxRetries} percobaan: ${lastError.message}`,
        ErrorCodes.UNKNOWN_ERROR,
        {
          title,
          artist,
  ...
  throw new Error("Unexpected code path reached");
}
...
export async function scrapeLyricsWithErrorHandling(title, artist, proxy) {
  try {
    ...
    return {
      success: true,
      data: await scrapeLyrics(title, artist, proxy, options),
